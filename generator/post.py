...
if False:
    for pcb in ('mainboard', 'support_board'):
        if not os.path.isfile('output/{}.normal.svg'.format(pcb)):
            continue

        tree = ET.parse('output/{}.normal.svg'.format(pcb))
        root = tree.getroot()
        ...
        with open('output/{}.traces.svg'.format(pcb), 'wb') as f:
            tree.write(f)
...
for pcb in ('support_board', 'mainboard'):
