...
uint16_t dimmed_brightness_fp(uint16_t max_brightness, uint16_t min_brightness) {
...
uint16_t estimate_illuminance_fp(uint16_t adc_samples) {
...
#define RESISTANCE_AT_10_LUX 15.0e3f

#define PULLUP_RESISTANCE 220.0e3f

#define GAMMA 0.7f

#define MAX_DIMMING_ILLUMINANCE 100.0f

// the same thing, but floating point because the new hardware does support that
float estimate_illuminance(uint16_t adc_samples) {
    if (adc_samples == 0) {
        return 100000.0f;
    }

    float normalized_samples = ((float)(MAX_ADC_SAMPLES - samples)) / (float)adc_samples;
...
uint16_t dimmed_brightness(uint16_t max_brightness, uint16_t min_brightness) {
    float clamped_illuminance = illuminance;
    if (clamped_illuminance > MAX_DIMMING_ILLUMINANCE) {
        clamped_illuminance = MAX_DIMMING_ILLUMINANCE;
    }

    float diff_brightness = max_brightness - min_brightness;
    uint16_t interpolated_brightness = (uint16_t)((diff_brightness * clamped_illuminance) / MAX_DIMMING_ILLUMINANCE) + min_brightness;
    return interpolated_brightness;
}
